...
class HeightCurveMaleYearsTestCase(APITestCase):
    """
    Test to show the height-based growth curve for males aged 3 to 20 years
    """
    ...
    def setUp(self):
        ...
        self.url = reverse('height:curve-male-years')
    ...
    def test_graphic_result_positive(self):
        ...
        url = reverse('height:curve-result')
        graphic = self.get_graphic()
        result = graphic.result(100.52, 3)
        response = self.client.post(url, data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(result, response.data['result'])
        self.assertTrue(response.data['result'] > 0)
